...
import { ArrowLeft, ShoppingCart, Star, ShieldCheck, Leaf } from "lucide-react";
...
export function ProductDetails() {
  ...
  useEffect(() => {
    async function fetchProduct() {
      if (!id) return;
      // Try local fallback first for instant render
      ...
      try {
        ...
        // silently fall through — not found
      } finally {
        setLoading(false);
      }
    }
  ...
  if (!product)
    return (
      <div className="pt-32 flex flex-col items-center justify-center min-h-screen bg-brand-earth gap-4">
        <p className="text-xl font-black uppercase tracking-tighter text-brand-green">Product not found.</p>
        <Link to="/shop" className="text-sm font-bold uppercase tracking-widest text-brand-green/50 hover:text-brand-green transition-colors">
          ← Back to Menu
        </Link>
      </div>
    );
  ...
  const trust = [
    {
      icon: ShieldCheck,
      title: "Lab Tested",
      sub: "Quality Verified",
    },
    {
      icon: Leaf,
      title: "Organic",
      sub: "No Synthetic Pesticides",
    },
  ];
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-10">
              <div className="flex items-center border border-brand-green/10 rounded-full bg-white p-1.5 shadow-sm">
                ...
                <span className="w-10 sm:w-12 text-center font-black text-brand-green text-base">
                  {quantity}
                </span>
                <button
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-3 bg-brand-green text-brand-earth py-4 sm:py-5 rounded-full text-[13px] font-black uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-transform shadow-lg shadow-brand-green/20"
              >
                Add to Cart
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-8 border-t border-brand-green/8">
              {trust.map(({ icon: Icon, title, sub }) => (
                <div key={title} className="flex gap-3 items-start">
                  ...
                  <div>
                    <h4 className="font-black uppercase tracking-tighter text-sm mb-0.5 text-brand-dark">
                      {title}
                    </h4>
                    <p className="text-[10px] text-brand-dark/35 font-bold uppercase tracking-widest">
                      {sub}
                    </p>
                  </div>
                </div>
